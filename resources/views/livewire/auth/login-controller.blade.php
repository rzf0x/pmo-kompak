<div id="auth-left">
    ...
    <h1 class="auth-title">
        Project Manajemen Office💼
        <span class="fs-3 text-capitalize d-block">
            Sistem Informasi Manajemen Proyek <strong>Kompak Team Agency</strong>
        </span>
    </h1>
    ...
    <form wire:submit="login" class="w-100">
        <div class="mb-3">
            ...
            <input class="form-control" required wire:model="email" type="email" placeholder="admin@gmail.com" />
            @error('email')
        ...
        <div class="mb-3">
            ...
            <input class="form-control" required wire:model="password" type="password" placeholder="******" />
            @error('password')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        @if (session()->has('toast_error'))
            <div wire:poll class="alert alert-danger" role="alert">
                <b>Gagal login!</b> {{ session('toast_error') }}
            </div>
        @endif
        @if (session()->has('toast_success'))
            <div wire:poll class="alert alert-success z-3" role="alert">
                <b>Berhasil login!</b> {{ session('toast_success') }}
            </div>
        @endif
        <button type="submit" class="mb-2 btn btn-primary w-100">Masuk</button>
    </form>
</div>
